...
if __name__ == '__main__':
    ...
    search_query = "@FeverTreeMixers"
    rate_delay_seconds = 0
    error_delay_seconds = 5

    # Example of using TwitterSearch
    # dropped for now, prevents the slicer dates from working
##    twit = TwitterSearchImpl(rate_delay_seconds, error_delay_seconds, None)
##    twit.search(search_query)

    # Example of using TwitterSlice
    select_tweets_since = datetime.datetime.strptime("2020-02-01", '%Y-%m-%d')
    select_tweets_until = datetime.datetime.strptime("2020-04-01", '%Y-%m-%d')
    threads = 1 # pulled back to 1 from 10, missing tweets otherwise
...
# excel
wb.save('fevertreetwitter20200331.csv')
